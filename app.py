...
def get_environmental_advice(location):
    ...
    if not api_key or api_key == 'YOUR_API_KEY_HERE':
        return {"title": "Environmental Advisor", "message": "Weather API key not set."}
    try:
        ...
        if "rain" in desc:
            return {"title": "Weather Alert", "message": f"It's raining in {location}. A perfect day for an indoor workout!"}
        ...
        return {"title": "Environmental Advisor", "message": f"Could not find weather for '{location}'."}
...
def analyze_mind_body_connection(health_df, mood_df):
    if len(health_df) < 5 or len(mood_df) < 5:
        return None
...
def find_optimal_slots(schedule, hobbies):
    ...
    for event in schedule:
        start_idx = int((event.start_hour - 8) * 2)
        end_idx = int((event.end_hour - 8) * 2)
        if 0 <= start_idx < 28 and 0 <= end_idx <= 28:
            slots[start_idx:end_idx] = 0
    best_workout_slot = -1
    best_workout_score = -1
    best_hobby_slots = []
    free_indices = np.where(slots == 1)[0]
    for i in free_indices:
        hour = 8 + i * 0.5
        workout_score = 1 - abs(hour - 16) / 8
        if workout_score > best_workout_score:
            best_workout_score = workout_score
            best_workout_slot = hour
    if hobbies and len(free_indices) > 2:
        for i in free_indices:
            ...
            if hour != best_workout_slot and (hour < 12 or hour > 18):
                ...
                if len(best_hobby_slots) >= len(hobbies):
                    break
...
def dashboard():
    ...
    insights = {}
    if health_data:
        health_df = pd.DataFrame([d.__dict__ for d in health_data])
...
def nutrition():
    ...
    return render_template('nutrition.html', tdee=tdee, calories_consumed=calories_consumed,
                           calories_burned=calories_burned, net_calories=net_calories, food_logs=food_today)
...
def profile():
    if request.method == 'POST':
        if 'event_name' in request.form:
            new_event = ScheduleEvent(event_name=request.form['event_name'], start_hour=float(request.form['start_hour']), end_hour=float(request.form['end_hour']), user_id=current_user.id)
            db.session.add(new_event)
            flash('Event added!', 'success')
        elif 'hobby_name' in request.form:
            new_hobby = Hobby(name=request.form['hobby_name'], user_id=current_user.id)
...
def add_data():
    try:
        ...
        if not data:
            data = HealthData(user_id=current_user.id, date=form_date)
            db.session.add(data)
        data.steps = int(request.form['steps'])
        data.sleep = float(request.form['sleep'])
        data.heart_rate = int(request.form['heart_rate'])
        data.calories_burned = int(request.form['calories_burned'])
        flash('Data logged successfully!', 'success')
        db.session.commit()
    except Exception as e:
        db.session.rollback()
        flash(f'Error logging data: {e}', 'error')
    return redirect(url_for('dashboard'))
...
def log_mood():
    ...
    if log:
        log.mood_score = mood_score
    else:
        db.session.add(MoodLog(user_id=current_user.id, date=date.today(), mood_score=mood_score))
    db.session.commit()
...
def simulate_day():
    ...
    if HealthData.query.filter_by(user_id=current_user.id, date=new_date).first():
        flash(f'Data for {new_date.strftime("%b %d")} already exists.', 'info')
        return redirect(url_for('dashboard'))
    new_data = HealthData(
...
def log_food():
    ...
    try:
        ...
        flash('The AI could not analyze that meal. Please try again.', 'error')
...
if __name__ == '__main__':
    with app.app_context():
        ...
        if not Badge.query.first():
            badges = [
                Badge(name="First Steps", icon="👟", description="Logged your first day of data."),
                Badge(name="Active Week", icon="🔥", description="Met a 7-day activity streak."),
                Badge(name="Sleep Champion", icon="😴", description="Achieved a week of ideal sleep."),
                Badge(name="Mindful Master", icon="🧘", description="Logged your mood for 7 consecutive days.")
            ]
